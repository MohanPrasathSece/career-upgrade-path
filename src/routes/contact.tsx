...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      { title: "Contact Career Upgrade - Speak With Admissions" },
      { name: "description", content: "Get in touch with our friendly UK admissions team. Phone, WhatsApp, email and a quick contact form to start your dental nursing journey." },
      { property: "og:title", content: "Contact Career Upgrade" },
...
function Contact() {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", course: "Dental Nursing - 1 Year", message: "" });
      ...
      <Section>
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {info.map((c) => {
            ...
            return c.href ? (
              <a key={c.label} href={c.href} className="rounded-2xl border border-border bg-card p-6 shadow-soft transition hover:-translate-y-1 hover:shadow-card hover:border-primary/40">
                {inner}
              </a>
            ) : (
            ...
            <form onSubmit={onSubmit} className="mt-7 grid gap-4">
              ...
              {sent && (
                <div className="mt-2 flex items-center gap-3 rounded-xl border border-primary/30 bg-secondary p-4 text-sm font-medium text-primary">
                  <CheckCircle2 className="h-5 w-5" /> Thanks - we've received your enquiry and will reply shortly.
                </div>
              )}
            </form>
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="rounded-3xl border border-border bg-gradient-soft p-7 shadow-soft">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Our admissions team replies within 1 working hour during opening times. For urgent enquiries, message us on WhatsApp.
              </p>
              <ul className="mt-5 space-y-2 text-sm">
                ...
                <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-primary" /> No obligation, ever</li>
              </ul>
            </div>
          </div>
        </div>
