...
export const Route = createFileRoute("/courses")({
  head: () => ({
    meta: [
      ...
      { property: "og:url", content: "https://careerupgradedentalnursingschool.co.uk/courses" },
    ...
    links: [{ rel: "canonical", href: "https://careerupgradedentalnursingschool.co.uk/courses" }],
  }),
  component: Courses,
});
...
const learn = [
  {
    ...
    desc: "Chairside support, instruments and clinical workflows.",
  },
  {
    icon: Heart,
    title: "Patient Care",
    desc: "Compassionate communication and patient experience.",
  },
  {
    icon: ShieldCheck,
    title: "Infection Control",
    desc: "Decontamination, sterilisation and clinical safety.",
  },
  {
    icon: Smile,
    title: "Oral Health Education",
    desc: "Educate patients on prevention and oral wellbeing.",
  },
  {
    icon: BriefcaseBusiness,
    title: "Dental Practice Support",
    desc: "Practice admin, records and team workflows.",
  },
  {
    icon: GraduationCap,
    title: "Clinical Skills",
    desc: "Hands-on competencies built through real placements.",
  },
];
        ...
        <div className="rounded-[2rem] border border-border bg-card p-8 shadow-card md:p-12">
          <div className="grid items-center gap-10 lg:grid-cols-[1.1fr_1fr]">
            <div>
              ...
              <Link
                to="/contact"
                className="mt-7 inline-flex items-center gap-2 rounded-full bg-gradient-primary px-7 py-3.5 text-sm font-semibold text-primary-foreground shadow-soft hover:shadow-glow hover:-translate-y-0.5 transition"
              >
      ...
      <Section>
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {learn.map((l) => (
            <div
              key={l.title}
              className="group rounded-2xl border border-border bg-card p-7 shadow-soft transition hover:-translate-y-1 hover:shadow-card hover:border-primary/40"
            >
        ...
        <Section>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-muted-foreground">
              Our admissions team will help you find the right route, including government funding
              for eligible UK learners.
            </p>
          </div>
          <div className="mt-12 grid gap-5 md:grid-cols-2">
            {funding.map((f) => (
              ...
              >
                ...
                <span
                  className={`grid h-12 w-12 place-items-center rounded-xl ${f.gradient ? "bg-white/15 text-primary-foreground" : "bg-secondary text-primary"}`}
                >
                  <f.icon className="h-5 w-5" />
                </span>
                <h3 className="mt-5 font-display text-xl font-bold">{f.title}</h3>
                <p
                  className={`mt-2 text-sm leading-relaxed ${f.gradient ? "opacity-90" : "text-muted-foreground"}`}
                >
                  {f.desc}
                </p>
              </div>
        ...
        <div className="grid items-center gap-12 lg:grid-cols-2">
          ...
          <div className="rounded-3xl border border-border bg-card p-8 shadow-card">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              Your enrolment includes all the essentials so you can focus on learning, not
              logistics.
            </p>
            ...
            <div className="mt-5 border-t border-border pt-4 text-center">
              <p className="text-xs text-muted-foreground leading-relaxed">
                All learners need Hepatitis B vaccination before they can start working in dental
                practice.
              </p>
            </div>
          </div>
          ...
          <div className="relative grid items-center gap-6 md:grid-cols-[1.4fr_1fr]">
            <div>
              <h2 className="font-display text-2xl font-bold sm:text-3xl md:text-4xl">
                Apply Today & Upgrade Your Career
              </h2>
              <p className="mt-3 max-w-xl opacity-90">
                Free consultation, transparent pricing and a personal funding plan in one call.
              </p>
            </div>
